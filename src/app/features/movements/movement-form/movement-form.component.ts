        ...
        @if (loading()) {
            ...
            <form class="glass-card" [formGroup]="form" (ngSubmit)="onSubmit()">
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        <label for="reference_document_type"
                            >Tipo documento ref.</label
                        >
                        <input
                            id="reference_document_type"
                            type="text"
                            formControlName="reference_document_type"
                        />
                    </div>
                    <div class="form-group">
                        <label for="reference_document_number"
                            >Número documento ref.</label
                        >
                        <input
                            id="reference_document_number"
                            type="text"
                            formControlName="reference_document_number"
                        />
                    </div>
                </div>
    ...
    ngOnInit(): void {
        ...
        const loads = [
            ...
            ...(this.isEdit && this.editId
                ? [this.movementService.getById(this.editId)]
                : []),
        ];
        ...
        forkJoin(loads).subscribe({
            next: (results) => {
                ...
                if (this.isEdit && results[1]) {
                    const m =
                        results[1] as import("../../../core/models/movement.model").Movement;
                    this.form.patchValue({
                        movement_type:
                            m.movement_type.toUpperCase() as MovementType,
                        reason: m.reason,
                        supplier_id: m.supplier?.id ?? null,
                        reference_document_type:
                            m.reference_document_type ?? "",
                        reference_document_number:
                            m.reference_document_number ?? "",
                    });
