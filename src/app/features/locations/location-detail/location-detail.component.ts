...
@Component({
    ...
    template: `
        <app-page-header title="Detalle de ubicación">
            <a [routerLink]="['/admin/ubicaciones', id, 'editar']" class="btn"
                >Editar</a
            >
        ...
        @if (loading()) {
            ...
            <div class="glass-card">
                <div class="detail-row">
                    <span class="label">ID</span><span>{{ loc.id }}</span>
                </div>
                <div class="detail-row">
                ...
                <div class="detail-row">
                    <span class="label">Nivel</span><span>{{ loc.level }}</span>
                </div>
                <div class="detail-row">
                ...
                <div class="detail-row">
                    ...
                    ><span>{{ loc.createdAt | date: "dd/MM/yyyy HH:mm" }}</span>
                </div>
            </div>
        }
    `,
...
export class LocationDetailComponent implements OnInit {
    ...
    ngOnInit(): void {
        this.service.getById(this.id).subscribe({
            ...
            error: (err) => {
                this.toast.error(
                    extractErrorMessage(err, "No se pudo cargar la ubicación."),
                );
                this.loading.set(false);
            },
        });
